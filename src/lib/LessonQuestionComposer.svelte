<script lang="ts">
  ...
  const questionLength = $derived(Array.from(question).length);
  ...
  const theme = $derived(style.preset === "momento" ? "momento" : style.preset === "wired" ? "wired" : style.preset === "arcade" ? "arcade" : "neutral");
  const accent = $derived(style.preset === "wired" ? style.wiredColors.fallbackAccent : style.preset === "arcade" ? style.arcadeColors.text : style.preset === "yellow-drop" ? "#f5df26" : "#f06aaa");
</script>

<section class="composer {theme}" class:compact style={`--ask-accent:${accent};--wired-panel:${style.wiredColors.panel};--wired-wash:${style.wiredColors.wash};--arcade-panel:${style.arcadeColors.panel};--arcade-text:${style.arcadeColors.text}`} aria-label="Ask Nono">
  ...
  {:else}
    ...
    <form onsubmit={(event) => { event.preventDefault(); send(); }}>
      <input bind:this={input} bind:value={question} maxlength="800" placeholder="Ask about meaning, grammar, tone, or culture…" aria-label="Question for Nono" />
      <button class="send" disabled={!question.trim()}>Ask Nono <span>→</span></button>
    </form>
    <small class="character-count" class:near-limit={questionLength > 720}>{800 - questionLength} characters left</small>
  {/if}
...
<style>
  .composer{position:relative;width:100%;height:100%;box-sizing:border-box;display:grid;grid-template-columns:minmax(0,1fr);grid-template-rows:auto auto auto 1fr;gap:7px;padding:13px 15px 14px;color:#f9f8fb;background:#17191eee;border:1px solid #ffffff40;border-radius:16px;box-shadow:0 16px 44px #0009;overflow:hidden;isolation:isolate;font-family:Inter,sans-serif}.character-count{position:absolute;right:15px;bottom:2px;color:#ffffff66;font-size:6px}.character-count.near-limit{color:#ffc06d}
  ...
  .arcade{border-radius:1px;background:linear-gradient(90deg,transparent,var(--arcade-panel) 7%,var(--arcade-panel) 93%,transparent);border:0;color:var(--arcade-text);text-shadow:0 0 5px color-mix(in srgb,var(--arcade-text) 65%,transparent);font-family:"Share Tech Mono",monospace}.arcade::before{background:repeating-linear-gradient(0deg,transparent 0 3px,#fff 4px 4px);opacity:.025}.arcade header b,.arcade .selected-line small{color:color-mix(in srgb,var(--arcade-text) 65%,transparent)}.arcade .selected-line{border-color:var(--arcade-text);background:#0005}.arcade .prompt-row button,.arcade input{font-family:"Share Tech Mono",monospace;border-color:color-mix(in srgb,var(--arcade-text) 55%,transparent);color:var(--arcade-text)}.arcade .send{background:var(--arcade-text);color:#111}
  .resume-media{position:absolute;left:14px;bottom:2px;border:0;background:transparent;color:var(--ask-accent);font-size:7px;font-weight:800;text-decoration:underline}.compact{height:100%;min-height:0;display:block;padding:4px 34px 4px 5px;border-radius:10px;box-shadow:none}.compact header,.compact .prompt-row{display:none}.compact form{height:100%}.compact .thinking-state{height:100%}.compact .close{right:5px;top:50%;width:26px;height:26px;transform:translateY(-50%)}
