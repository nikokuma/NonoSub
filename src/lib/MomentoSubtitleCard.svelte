  ...
  <span class="cutout">
    ...
    {#if showTranslation}
      <span class="translation-card" class:waiting={!translation}>
        {#key translation}<span>{translation || "Translation catching up…"}</span>{/key}
      </span>
    {/if}
  </span>
</button>
...
<style>
  ...
  .translation-card > span { display: block; animation: target-text-in 150ms ease-out both; }
  ...
  @media(prefers-reduced-motion:no-preference) { .speaker-tab { animation: accent-snap 70ms ease-out both; } .source-card { animation: source-wipe 160ms cubic-bezier(.2,.8,.2,1) both; } .translation-card { animation: target-wipe 190ms 40ms cubic-bezier(.2,.8,.2,1) both; } }
