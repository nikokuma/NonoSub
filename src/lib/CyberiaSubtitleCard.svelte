<script lang="ts">
  ...
  const colors = $derived(style.cyberiaColors);
...
<style>
  ...
  @media(prefers-reduced-motion:no-preference){.speaker-tab{animation:tab-snap 70ms ease-out both}.selected-panel{animation:panel-wipe 150ms cubic-bezier(.2,.8,.2,1) both}.translation>span{animation:translation-in 180ms 35ms ease-out both}}
