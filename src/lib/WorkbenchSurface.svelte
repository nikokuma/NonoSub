<script lang="ts">
  ...
  async function startLive() {
    ...
    try {
      ...
      await invoke("start_live_capture", { languages: preferences.languages });
  ...
  <main>
    <section class="command-deck">
      ...
      <section class="language-panel" id="languages" tabindex="-1">
        ...
        <label>Nono explains in<select bind:value={preferences.languages.explanation} onchange={persist}>{#each LANGUAGE_OPTIONS.filter(([code]) => code !== "auto") as language}<option value={language[0]}>{language[1]}</option>{/each}</select></label>
    ...
    <aside class="transcript-rail">
      <div class="rail-head"><div><span class="eyebrow">CURRENT SESSION</span><h2>Transcript</h2></div><span>{session.segments.length} LINES</span></div>
