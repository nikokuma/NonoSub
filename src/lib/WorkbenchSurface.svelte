      ...
      <div class="transcript">
        ...
        {#each session.segments as segment}
          ...
          <button class:current={selectedId === segment.id} onclick={() => selectLine(segment)} disabled={segment.isProvisional}>
            <time>{formatTime(segment.startMs)}</time><div><strong style={`color:${speaker?.color ?? "#79e9cb"}`}>{speaker?.displayName ?? (segment.origin === "live" ? "Live Audio" : "Speaker")}</strong><p>{segment.sourceText}</p>{#if session.processingMode !== "original_only"}<span>{segment.translationText ?? "Translating…"}</span>{/if}</div>
          </button>
        {/each}
      </div>
