<script lang="ts">
  ...
  import type { ApiConfigurationStatus, SessionState, SpeakerProfile, SubtitlePreset, SubtitleSegment } from "./contracts";
  ...
  async function removeApiKey() {
    ...
    try {
      ...
      onboarding = true;
      apiMessage = "API key removed from Keychain.";
    } catch (error) {
      apiMessage = errorMessage(error);
    }
  }
  ...
  <main>
    <section class="command-deck">
      ...
      <section class="language-panel" id="languages" tabindex="-1">
        <div><span class="eyebrow">LANGUAGE ROUTING</span><h2>Model-supported languages → your chosen language</h2></div>
        ...
        <label class="live-engine">Live translation engine<select bind:value={preferences.sync.translationEngine} disabled={preferences.processingMode === "original_only"} onchange={() => void persist({ sync: { translationEngine: preferences.sync.translationEngine } })}><option value="realtime">Realtime — Fast</option><option value="transcript_locked">Transcript-Locked — Accurate (Experimental)</option></select></label>
...
<style>
  ...
  @media(max-width:1050px){.language-panel .processing,.language-panel .explanation,.language-panel .live-timing,.language-panel .live-engine,.language-note,.next-session{grid-column:auto}}
</style>
